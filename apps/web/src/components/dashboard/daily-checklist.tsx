    ...
    <aside className="w-full md:w-80 flex flex-col gap-6 order-3">
      <section className="bg-surface-container-low rounded-xl p-6 shadow-none flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-1">
          {checklist.map((item) => (
            <label key={item.id} className={`flex items-center gap-4 p-3 rounded-lg hover:bg-surface-container-high transition-colors cursor-pointer group ${item.strike ? 'bg-surface-container-high/50' : ''}`}>
              <div className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${
                item.completed ? 'border-primary/40 bg-primary/10' : 'border-outline-variant/30 group-hover:border-outline-variant'
              }`}>
                {item.completed && (
        ...
        <div className="mt-4 pt-6 border-t border-outline-variant/10">
          <p className="text-[11px] font-bold text-on-surface-variant uppercase tracking-[0.2em] mb-4">Focus Quality</p>
          <div className="h-1.5 w-full bg-surface-container-high rounded-full overflow-hidden">
            <div className="h-full bg-secondary w-3/4 rounded-full"></div>
          </div>
          <p className="mt-2 text-[10px] text-on-surface-variant text-right italic">"Flow state detected in last 2 sessions"</p>
        </div>
      </section>
